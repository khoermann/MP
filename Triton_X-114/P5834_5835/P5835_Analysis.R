# Results for P5835: 
# P5835: 30 Mio wt cells subjected to phase separation +FASP, biological replicate of P5834
# Triton X-114, detergent phase, FASP, 1-D shotgun, 2 injections

setwd("~/MP_RStudio_May2014/Triton_X-114/P5834_5835/"); getwd();

# Read in the file created with "P5834_results.R" containing the GOCC annotation =========================================
annot <- read.csv2("~/MP_RStudio_May2014/Triton_X-114/P5834_5835/P5835.GOterms.csv", header=T, stringsAsFactors=F)
dim(annot)
annot$norm.sc <- annot$global.spectra.count/sum(annot$global.spectra.count)   # add a column with the normalized sc

# Look for SLCs -------------------------------------
length(unique(grep("SLC", annot$Gene)))   # 67 SLCs
length(unique(grep("ABC", annot$Gene)))   # 9 ABCs

slc <- annot[grep("SLC", annot$Gene),]
abc <- annot[grep("ABC", annot$Gene),]
write.csv2(rbind(slc, abc), file="SLCsABCs_P5835.csv")  
...
P5835.itm <- subset(annot, matched_mask)   # No matches for this subset
dim(P5835.itm)

write.csv2(matches.pm.only, file="PMOnly_P5835.csv")
write.csv2(matches.pm, file="PM_P5835.csv")
write.csv2(non_PM_mem, file="Non_PM_mem_P5835.csv")
write.csv2(other, file="Other_P5835.csv")
write.csv2(mem, file="Membrane_P5835.csv")


# Plot sc distribution ========================================
pdf("~/MP_RStudio_May2014/Triton_X-114/P5834_5835/GSC_distrib_P5835.pdf")
...
pdf("~/MP_RStudio_May2014/Triton_X-114/P5834_5835//sc_distrib_vertical_P5835.pdf")
boxplot(log10(mem$global.spectra.count), log10(other$global.spectra.count), 
        ...
        col=c("red", "grey"), 
        main="Global Spectra Count distribution P5835")
dev.off()

# Horizontal version of the boxplot
pdf("~/MP_RStudio_May2014/Triton_X-114/P5834_5835//sc_distrib_horizontal_P5835.pdf")
boxplot(log10(mem$global.spectra.count), log10(other$global.spectra.count), 
        horizontal=T,
        ylab="Log10(Global Spectra Counts)", 
        names=c("Membrane", "Non-membrane"),
