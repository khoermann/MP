# Results for P5834: 
# P5834: 30 Mio wt cells subjected to phase separation +FASP
...
annot <- read.csv2("~/MP_RStudio_May2014/Triton_X-114/P5834_5835/P5834.GOterms.csv", header=T, stringsAsFactors=F)
dim(annot)
annot$norm.sc <- annot$global.spectra.count/sum(annot$global.spectra.count)   # add a column with the normalized sc

# Look for SLCs -------------------------------------
length(unique(grep("SLC", annot$Gene)))   # 64 SLCs
length(unique(grep("ABC", annot$Gene)))   # 10 ABCs

slc <- annot[grep("SLC", annot$Gene),]
abc <- annot[grep("ABC", annot$Gene),]
write.csv2(rbind(slc, abc), file="SLCsABCs_P5834.csv")  
...
P5834.itm <- subset(annot, matched_mask)   # No matches for this subset
dim(P5834.itm)

write.csv2(matches.pm.only, file="PMOnly_P5834.csv")
write.csv2(matches.pm, file="PM_P5834.csv")
write.csv2(non_PM_mem, file="Non_PM_mem_P5834.csv")
write.csv2(other, file="Other_P5834.csv")
write.csv2(mem, file="Membrane_P5834.csv")


# Plot sc distribution ========================================
pdf("~/MP_RStudio_May2014/Triton_X-114/P5834_5835/GSC_distrib_P5834.pdf")
...
pdf("~/MP_RStudio_May2014/Triton_X-114/P5834_5835//sc_distrib_vertical_P5834.pdf")
boxplot(log10(mem$global.spectra.count), log10(other$global.spectra.count), 
        ...
        col=c("red", "grey"), 
        main="Global Spectra Count distribution P5834")
dev.off()

# Horizontal version of the boxplot
pdf("~/MP_RStudio_May2014/Triton_X-114/P5834_5835//sc_distrib_horizontal_P5834.pdf")
boxplot(log10(mem$global.spectra.count), log10(other$global.spectra.count), 
        horizontal=T,
        ylab="Log10(Global Spectra Counts)", 
        names=c("Membrane", "Non-membrane"),
