# Results for P5768: 
# P5768: 120 Mio KBM7 wt cells labeled and processed according to Aminooxybiotin protocol

setwd("~/MP_RStudio_May2014/Aminooxybiotin/P5767_P5768/P5768/"); getwd();

# Read in the file created with "P5768_results.R" containing the GOCC annotation =========================================
annot <- read.csv2("~/MP_RStudio_May2014/Aminooxybiotin/P5767_P5768/P5768/P5768.GOterms.csv", header=T, stringsAsFactors=F)
dim(annot)
...
write.csv2(rbind(slc, abc), file="SLCsABCs_P5768.csv")  


# Divide into GOCC subgroups ----------------------------------
mem <- annot[grep('membrane', annot$Go.Cc), ]   # 81 proteins
dim(mem)

# PM only
PM.only <- c('plasma membrane', 'cell surface', 'cell membrane')
matches.pm.only <- annot[grep(paste(PM.only, collapse='|'), annot$Go.Cc), ]; dim(matches.pm.only)  # 79 matches
...
P5768.itm <- subset(annot, matched_mask)   # No matches for this subset
dim(P5768.itm)

write.csv2(matches.pm.only, file="PMOnly_P5768.csv")
write.csv2(matches.pm, file="PM_P5768.csv")
write.csv2(non_PM_mem, file="Non_PM_mem_P5768.csv")
write.csv2(other, file="Other_P5768.csv")


# Plot sc distribution ========================================
pdf("~/MP_RStudio_May2014/CellSurfaceExtractionKit/P5768/GSC_distrib_P5768.pdf")
...
pdf("~/MP_RStudio_May2014/CellSurfaceExtractionKit/P5768/sc_distrib_vertical_P5768.pdf")
...
pdf("~/MP_RStudio_May2014/CellSurfaceExtractionKit/P5768/sc_distrib_horizontal_P5768.pdf")
boxplot(log10(matches.pm$global.spectra.count), log10(non_PM_mem$global.spectra.count), log10(other$global.spectra.count), 
        horizontal=T,
