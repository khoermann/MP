...
setwd("/slow/home/khoermann/MP/Aminooxybiotin/id.csv"); getwd(); 
id.files <- Sys.glob(c("/slow/home/khoermann/MP/Aminooxybiotin/id.csv/P5767-[34].id.csv",
                       "/slow/home/khoermann/MP/Aminooxybiotin/id.csv/P5768-[34].id.csv"))
...
write.csv2(result.df, file='201400324_Isobar_P5767.P5768.csv')

result.df <- read.csv2("201400324_Isobar_P5767.P5768.csv", header=TRUE, stringsAsFactors=F)   # 465 proteins
...
DF2 <- aggregate(DF[2], DF[-2], 
                 FUN = function(X) paste(unique(X), collapse=", "))   # 458 proteins annotated
colnames(DF2) <- c("protein.acs", "Go.Cc")

combi <- merge(result.df[,2:15], DF2, by="protein.acs")     # Combine Isobar data and annotations into one dataframe
write.csv2(combi, file="20140324.Aminooxy.P5767.P5768-34.GOterms.csv")
