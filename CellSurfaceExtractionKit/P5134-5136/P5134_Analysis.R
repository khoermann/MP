# Results for P5134: 
# P5134: 40 Mio wt cells labeled, Cell Surface Extraction Kit
# Cell Surface Protein Extraction Kit, SDS elution, 1-D shotgun, 2 injections

setwd("~/MP_RStudio_May2014/CellSurfaceExtractionKit/P5134-5136/"); getwd();

# Read in the file created with "P5134_results.R" containing the GOCC annotation =========================================
annot <- read.csv2("~/MP_RStudio_May2014/CellSurfaceExtractionKit/P5134-5136/P5134.GOterms.csv", header=T, stringsAsFactors=F)
dim(annot)
annot$norm.sc <- annot$global.spectra.count/sum(annot$global.spectra.count)   # add a column with the normalized sc

# Look for SLCs -------------------------------------
length(unique(grep("SLC", annot$Gene)))   # 23 SLCs
length(unique(grep("ABC", annot$Gene)))   # 11 ABCs

slc <- annot[grep("SLC", annot$Gene),]
abc <- annot[grep("ABC", annot$Gene),]
write.csv2(rbind(slc, abc), file="SLCsABCs_P5134.csv")  
...
P5134.itm <- subset(annot, matched_mask)   # No matches for this subset
dim(P5134.itm)

write.csv2(matches.pm.only, file="PMOnly_P5134.csv")
write.csv2(matches.pm, file="PM_P5134.csv")
write.csv2(non_PM_mem, file="Non_PM_mem_P5134.csv")
write.csv2(other, file="Other_P5134.csv")


# Plot sc distribution ========================================
pdf("~/MP_RStudio_May2014/CellSurfaceExtractionKit/P5134/GSC_distrib_P5134.pdf")
...
pdf("~/MP_RStudio_May2014/CellSurfaceExtractionKit/P5134/sc_distrib_vertical_P5134.pdf")
...
pdf("~/MP_RStudio_May2014/CellSurfaceExtractionKit/P5134/sc_distrib_horizontal_P5134.pdf")
boxplot(log10(matches.pm$global.spectra.count), log10(non_PM_mem$global.spectra.count), log10(other$global.spectra.count), 
        horizontal=T,
