...
# P6287: 40 Mio wt cells labeled, Cell Surface Extraction Kit, washing with regular wash buffer, Biotin elution with solution containing no detergent components at all!
setwd("~/MP_RStudio_May2014/CellSurfaceExtractionKit//id.csv"); getwd(); 
id.files <- Sys.glob(c("P6287-[12].id.csv"))
...
dim(result.df)  # 671 proteins detected
setwd("~/MP_RStudio_May2014/CellSurfaceExtractionKit/P6287/"); getwd(); 
write.csv2(result.df, file='Isobar_P6287.csv')
...
dim(combi)
write.csv2(combi, file="P6287.GOterms.csv")       # Contains 663 entries
