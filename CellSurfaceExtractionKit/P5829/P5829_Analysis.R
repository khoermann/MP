# Results for P5829: 
# Negative control for P5828. Unlabeled control
# Cell Surface Protein Extraction Kit, SDS elution, + TX-114 phase separation, DETERGENT phase, 1-D shotgun, 2 injections

setwd("~/MP_RStudio_May2014/CellSurfaceExtractionKit/P5829/"); getwd();

# Read in the file created with "P5829_results.R" containing the GOCC annotation =========================================
annot <- read.csv2("~/MP_RStudio_May2014/CellSurfaceExtractionKit/P5829//P5829.GOterms.csv", header=T, stringsAsFactors=F)
dim(annot)
annot$norm.sc <- annot$global.spectra.count/sum(annot$global.spectra.count)   # add a column with the normalized sc

# Look for SLCs -------------------------------------
length(unique(grep("SLC", annot$Gene)))   # 4 SLCs
length(unique(grep("ABC", annot$Gene)))   # 1 ABCs

slc <- annot[grep("SLC", annot$Gene),]
abc <- annot[grep("ABC", annot$Gene),]
write.csv2(rbind(slc, abc), file="SLCsABCs_P5829.csv")  


# Divide into GOCC subgroups ----------------------------------
mem <- annot[grep('membrane', annot$Go.Cc), ]   # 174 proteins
dim(mem)

# PM only
PM.only <- c('plasma membrane', 'cell surface', 'cell membrane')
matches.pm.only <- annot[grep(paste(PM.only, collapse='|'), annot$Go.Cc), ]; dim(matches.pm.only)  # 86 matches

# Extended PM
toMatch <- c('plasma membrane', 'cell surface', 'extracellular', 'cell membrane')
matches.pm <- annot[grep(paste(toMatch, collapse='|'), annot$Go.Cc), ]    # Gives 263 matches
...
P5829.itm <- subset(annot, matched_mask)   # No matches for this subset
dim(P5829.itm)

write.csv2(matches.pm.only, file="PMOnly_P5829.csv")
write.csv2(matches.pm, file="PM_P5829.csv")
write.csv2(non_PM_mem, file="Non_PM_mem_P5829.csv")
write.csv2(other, file="Other_P5829.csv")


# Plot sc distribution ========================================
pdf("~/MP_RStudio_May2014/CellSurfaceExtractionKit/P5829/GSC_distrib_P5829.pdf")
...
pdf("~/MP_RStudio_May2014/CellSurfaceExtractionKit/P5829/sc_distrib_vertical_P5829.pdf")
...
pdf("~/MP_RStudio_May2014/CellSurfaceExtractionKit/P5829/sc_distrib_horizontal_P5829.pdf")
boxplot(log10(matches.pm$global.spectra.count), log10(non_PM_mem$global.spectra.count), log10(other$global.spectra.count), 
        horizontal=T,
