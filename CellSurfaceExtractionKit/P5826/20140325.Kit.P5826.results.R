...
id.files <- Sys.glob(c("P5826-[12].id.csv"))
...
write.csv2(result.df, file='20140325_Isobar_P5826.csv')
...
write.csv2(DF2, file="20140325.Kit.P5826.GOterms.csv")
...
id.1 <- read.csv2("~/MP_RStudio_May2014/CellSurfaceExtractionKit/id.csv//P5826-1.id.csv", header=T, sep="\t", stringsAsFactors=F)
id.2 <- read.csv2("~/MP_RStudio_May2014/CellSurfaceExtractionKit/id.csv//P5826-2.id.csv", header=T, sep="\t", stringsAsFactors=F)
...
biot.1 <- gsub("[,-].*",'',biot.1) 
biot.2 <- gsub("[,-].*",'',biot.2)
total.biot <- sort(unique(c(biot.1, biot.2)))   # 162 proteins biotinylated (34% of PM proteins)
write.csv2(total.biot, "~/MP_RStudio_May2014/CellSurfaceExtractionKit/P5826/Biotinylated_ACs_P5826.csv")
