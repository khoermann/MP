# Results for P5833: 
# Negative control for P5832. Unlabeled control
# Cell Surface Protein Extraction Kit, Biotin elution, 1-D shotgun, 2 injections

setwd("~/MP_RStudio_May2014/CellSurfaceExtractionKit/P5833/"); getwd();

# Read in the file created with "P5833_results.R" containing the GOCC annotation =========================================
annot <- read.csv2("~/MP_RStudio_May2014/CellSurfaceExtractionKit/P5833//P5833.GOterms.csv", header=T, stringsAsFactors=F)
dim(annot)
...
write.csv2(rbind(slc, abc), file="SLCsABCs_P5833.csv")  


# Divide into GOCC subgroups ----------------------------------
mem <- annot[grep('membrane', annot$Go.Cc), ]   # 75 proteins
dim(mem)

# PM only
PM.only <- c('plasma membrane', 'cell surface', 'cell membrane')
matches.pm.only <- annot[grep(paste(PM.only, collapse='|'), annot$Go.Cc), ]; dim(matches.pm.only)  # 46 matches

# Extended PM
toMatch <- c('plasma membrane', 'cell surface', 'extracellular', 'cell membrane')
matches.pm <- annot[grep(paste(toMatch, collapse='|'), annot$Go.Cc), ]    # Gives 168 matches
...
P5833.itm <- subset(annot, matched_mask)   # No matches for this subset
dim(P5833.itm)

write.csv2(matches.pm.only, file="PMOnly_P5833.csv")
write.csv2(matches.pm, file="PM_P5833.csv")
write.csv2(non_PM_mem, file="Non_PM_mem_P5833.csv")
write.csv2(other, file="Other_P5833.csv")


# Plot sc distribution ========================================
pdf("~/MP_RStudio_May2014/CellSurfaceExtractionKit/P5833/GSC_distrib_P5833.pdf")
...
pdf("~/MP_RStudio_May2014/CellSurfaceExtractionKit/P5833/sc_distrib_vertical_P5833.pdf")
...
pdf("~/MP_RStudio_May2014/CellSurfaceExtractionKit/P5833/sc_distrib_horizontal_P5833.pdf")
boxplot(log10(matches.pm$global.spectra.count), log10(non_PM_mem$global.spectra.count), log10(other$global.spectra.count), 
        horizontal=T,
