...
# P6157: biotinylated, Biotin elution
setwd("H:/MP/PierceKit/dat/id.csv"); getwd(); 
id.files <- Sys.glob(c("P6157-[12].id.csv"))
...
write.csv2(result.df, file='Isobar_P6157.csv')
...
write.csv2(combi, file="P6157.GOterms.csv")       # Contains 722 entries
...
id.1 <- read.csv2("~/MP_RStudio_May2014/CellSurfaceExtractionKit/id.csv//P6157-1.id.csv", header=T, sep="\t", stringsAsFactors=F)
id.2 <- read.csv2("~/MP_RStudio_May2014/CellSurfaceExtractionKit/id.csv//P6157-2.id.csv", header=T, sep="\t", stringsAsFactors=F)
...
total.biot <- sort(unique(c(biot.1, biot.2)))   # 152 proteins biotinylated (39% of PM proteins)
write.csv2(total.biot, "~/MP_RStudio_May2014/CellSurfaceExtractionKit/P6157/Biotinylated_ACs_P6157.csv")
